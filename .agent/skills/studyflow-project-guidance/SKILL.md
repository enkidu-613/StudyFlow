...
3. **子代理推理强度梯度**：子代理按“推理强度从低到高”逐级提升——先用低推理强度的模型快速尝试，解决不了再换更高推理强度的模型，避免一开始就用昂贵/慢的模型，也不要长时间卡在低强度模型上：
   - 低强度：`deepseek v4 flash`（快速试错、简单改动）。
   - 中强度：`qwen3.8`（中等复杂度推理与重构）。
   - 高强度：`luna`、`gpt 5.6`、`kimi k3`（复杂架构、疑难排障、界面设计规划）。
...
- **前端界面规划**：由 `kimi k3` 子代理负责界面与交互设计，输出页面结构、状态流转和组件拆分方案。
- **前端实现执行**：由 `deepseek v4 flash` 负责落地规划，编写 Flutter 界面与状态代码。
- **实现出现问题时**：先由 `gpt 5.6` 和 `luna` 子代理依次修改修复。
- **再往后**：按同样的“规划 → 执行 → 修复”分工逐级轮换，直到问题解决；每级都要记录尝试与结果，避免重复劳动。
- **推理强度从低到高**：先让低推理强度模型（`deepseek v4 flash`）快速落地，遇阻再升到中（`qwen3.8`）、高（`gpt 5.6`、`luna`、`kimi k3`）强度模型处理复杂设计与疑难问题，避免一上来就用高成本模型。
